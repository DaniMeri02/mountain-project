...
import { getAllAreas, getArea, deleteArea, decrTileRefs } from './idb';
...
export { getAllAreas, getArea, putArea, deleteArea, putOverlays, getOverlays, incrTileRefs, decrTileRefs } from './idb';
export { fetchBundle, downloadArea } from './download';
export { isDrawing, startDrawMode, cancelDrawMode } from './draw-mode';
export { openArea, exitOfflineArea } from './area';
...
function attachListToggle(map: mapboxgl.Map): void {
  ...
  panel.addEventListener('click', (e) => {
    ...
    if (target.classList.contains('offline-open')) {
      ...
      removeArea(id).then(() => renderAreasList()).catch((err: unknown) => console.error(err));
    }
  });
}
...
async function renderAreasList(): Promise<void> {
...
function attachSaveButton(map: mapboxgl.Map): void {
  ...
  btn.addEventListener('click', () => {
    ...
    startDrawMode(map, async (bbox) => {
      ...
      try {
        ...
        window.alert(`Failed: ${(err as Error).message}`);
      }
    });
  });
}

export function initOfflineModule(map: mapboxgl.Map): void {
  injectControls();
  attachListToggle(map);
  attachSaveButton(map);
  renderAreasList().catch((err: unknown) => console.warn('renderAreasList failed', err));
}
